...
def load_images_from_folder(folder, name_list):
    images = []
...
train_set_name = ['bbyz', 'cjw', 'ddd', 'dj', 'gdr', 'jd', 'ls', 'lx', 'rzw', 'wj', 'wsy', 'xw', 'yt', 'zdx', 'zxb']
test_set_name = ['csb', 'sf']
img_obj_list_train = load_images_from_folder(image_path, train_set_name)

train_dataset = ImgDataset(img_obj_list_train)
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():  # 不需要梯度计算
            ...
            if mse < best_mse:
                best_mse = mse
                # 保存模型的权重
                torch.save(model.state_dict(), '/gjh/ECGtoBP/best_EfficientNet_weights_solution2.pth')
...
# 保存图表为文件
plt.savefig('/gjh/ECGtoBP/EfficientNet_solution_2_training_loss.png')

best_model = EfficientNetB0(num_classes=1)
best_model.load_state_dict(torch.load('/gjh/ECGtoBP/best_EfficientNet_weights_solution2.pth'))
...
img_obj_list_test = load_images_from_folder(image_path, test_set_name)

test_dataset = ImgDataset(img_obj_list_test)
